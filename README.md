...
Electronic Warfare (EW) systems rely on continuous monitoring of radar signals to detect, classify, and respond to potential aerial threats. The ability to identify abnormal radar behavior in real time is essential for maintaining situational awareness and supporting timely tactical decision-making. This project presents a Radar Signal Anomaly Detection System designed to simulate an Electronic Warfare environment using a network of multiple radar stations and machine learning techniques.

The proposed system simulates aircraft movement with both normal and anomalous flight behaviors within a radar surveillance region. Multiple radar stations continuously generate radar measurements such as Range, Bearing, Signal Power, Frequency, and Relative Velocity, with realistic Gaussian noise incorporated to emulate practical radar sensor uncertainties. These measurements represent the live radar observations received by an Electronic Warfare system.

Statistical features are extracted from the radar measurements using a sliding-window approach and are used to train an Isolation Forest model on normal radar behavior. During operation, the trained model identifies abnormal flight patterns such as zig-zag movement, sudden heading changes, extreme acceleration, suspicious loitering, altitude spikes, and evasive maneuvers by detecting deviations from the learned normal patterns.

To improve decision-making, the system incorporates a Threat Assessment Engine that combines the Isolation Forest prediction with signal-based and behavior-based analysis to compute a final threat score and classify the aircraft into LOW, MEDIUM, HIGH, or CRITICAL threat levels. The results are visualized through a real-time dashboard with graphical aircraft tracking, threat indicators, and voice alerts, demonstrating how machine learning can assist Electronic Warfare systems in detecting suspicious aerial activities and enhancing radar surveillance capabilities.
